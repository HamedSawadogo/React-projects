import React, { useState } from 'react';
import Car from './Car';

const CarItems = () => {

  const carList = [
    {
      name: "BMW M6",
      url:
        "https://mediapool.bmwgroup.com/cache/P9/201411/P90169551/P90169551-the-new-bmw-m6-coup-exterior-12-2014-600px.jpg",
      release_year: 2020
    },
    {
      name: "VW Polo",
      url:
        "https://cdn.euroncap.com/media/30740/volkswagen-polo-359-235.jpg?mode=crop&width=359&height=235",
      release_year: 2018
    },
    {
      name: "Audi S6",
      url:
        "https://www.motortrend.com/uploads/sites/5/2020/03/6-2020-audi-s6.jpg?fit=around%7C875:492.1875",
      release_year: 2020
    },
    {
      name: "BMW M2",
      url:
        "https://imgd.aeplcdn.com/0x0/cw/ec/37092/BMW-M2-Exterior-141054.jpg?wm=0",
      release_year: 2019
    },
    {
      name: "Audi A3",
      url: "https://cdn.motor1.com/images/mgl/BEooZ/s3/2021-audi-s3.jpg",
      release_year: 2019
    }
  ];

  const [date, setDate] = useState('2018');
  const [name, setName] = useState('Audi A3');

  return (
    <div>
      <select name="voitures" id="" onChange={(e) => setDate(e.target.value)}>
        <optgroup>
          <option value="2018">2018</option>
          <option value="2020">2020</option>
          <option value="2019">2019</option>
        </optgroup>
      </select>
      {/* input recherher une voiture par son nom */}
      <input type="search" name="" id="" value={name}
        onChange={(e) => setName(e.target.value)}
      />
      <div className="cars" style={{
        display: "flex",
        justifyContent: "center",
        alignItems: "center",
        flexWrap: "wrap"
      }}>
        {carList
          .filter((car) => car.release_year == date)
          .filter((car) => {
            return car.name.toLowerCase().includes(name.toLowerCase())
          })
          .map((car) => (
            <Car url={car.url} name={car.name} date={car.release_year} />
          ))}
      </div>
    </div>
  );
};

export default CarItems;